...
func main() {
	...
	// POST /api/agent/schedule-post  —  创建定时发布任务
	r.POST("/api/agent/schedule-post", func(c *gin.Context) {
		var req struct {
			UserID       int64    `json:"user_id"       binding:"required"`
		...
		ctx, cancel := context.WithTimeout(c.Request.Context(), 3*time.Second)
		defer cancel()
		resp, err := agentClient.CreateScheduledPost(ctx, &agentpb.CreateScheduledPostReq{
			UserId: req.UserID, Title: req.Title, Content: req.Content,
			ImageUrls: req.ImageURLs, Tags: req.Tags, ScheduleTime: req.ScheduleTime,
		})
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"code": 500, "error": err.Error()})
			return
		}
		c.JSON(http.StatusOK, gin.H{"code": 0, "data": gin.H{"task_id": resp.TaskId, "status": resp.Status}})
	})
